...
def check_battery_and_get_speed():
    ...
    # Critical Battery (> 55s) -> FLASHING BLUE
    if elapsed > 55000:
        dmd.stop()
        set_leds(0, 0, 1) # Turn BLUE on immediately
        
        # Blink BLUE for 5 seconds
        end = ticks_ms() + 5000
        while ticks_ms() < end:
            set_leds(0, 0, 1) # Blue ON
            sleep(0.05)
            set_leds(0, 0, 0) # OFF
            sleep(0.05)
        sys.exit()

    # Low Battery (> 45s) -> RED MODE
    elif elapsed > 45000:
        return 0.25, True # 50% Speed, Force Low Power = True
        
    # Normal Mode (< 45s) -> GREEN MODE
    else:
        return 0.5, False # Normal Speed, Force Low Power = False
...
if __name__ == "__main__":
    try:
        ...
        while True:
            ...
            while True:
                speed, force_low_power = check_battery_and_get_speed()
                
                # LED Logic
                if force_low_power:
                    set_leds(1, 0, 0) # RED (Low Battery)
                else:
                ...
                d = sensor.distance
                if d is not None:
                    print(f"Distance: {d:.2f} m") # <--- PRINTING DISTANCE
                    if 0.15 <= d <= 0.4:
                        break
                sleep(0.05)
            ...
            # Stop (Hold Green)
            dmd.stop()
            set_leds(0, 65535, 0) # <--- THIS LINE MAKES IT GREEN DURING PAUSE
            sleep(0.5)
